...
**Version:** 3.1 | **Created:** 2026-07-16 | **Updated:** 2026-07-22
**Status:** ACTIVE
**Owner:** Rob + Max
...
|------|----------|-----------|--------|
| 2026-07-22 | **ACCESS model (Rob dump 7.21.26-1):** no login prompts pre-rollout (basic auth REMOVED — site currently OPEN, Rob accepted exposure trade); rollout = admin portal creates users + Google OAuth sign-in whose consent also grants Gmail scopes (email capture + send-as-rep come free with onboarding); Rob gets "View as rep" impersonation; full rep-POV mockup REQUIRED before rollout | Rob verbatim in docs/plans/sources/7.21.26-1.md | Rob |
| 2026-07-22 | **COMMS architecture settled:** Twilio = telephony rails; **Vapi = agent layer standard** (AIDRE confirmed built on Vapi — 67 refs in its code); LiveKit = self-host hedge if scale economics demand; digital receptionist = AIDRE consumed by the CRM (product lanes stay separate); inbound flow: ring rep → miss → AI answers WITH caller's CRM record pre-loaded via our lookup API | Rob + code verification 2026-07-22 | Rob/Max |
| 2026-07-22 | **Comms data lake principle:** ALL channels (rep calls, AIDRE inbound, AIVA chats, rep email) land in ONE corpus (activities→transcripts→embeddings); Leaky-Bucket-3 products are producers AND consumers; end state = live on-call assist (Vapi live events make it feasible) | Rob dump | Rob |
...
| 2026-07-22 | **Modular lanes: designed, NOT started** — requires Rob GO + quadruple-check protocol (CI prerequisite now met); Architecture Atlas artifact = living doc, refreshed after every milestone (driver duty) — "ZERO mixups… NOT NEGOTIABLE" | Rob | Rob |
