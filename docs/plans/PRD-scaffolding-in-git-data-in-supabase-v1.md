...
**Status:** PLANNING · **Owner:** Rob + Max · **Project:** MLE ROB Dashboard · **Type:** technical
...
graph LR
  ...
  P1["Phase 1<br/>Stop the bleeding"]:::ready
...
- [ ] [Max] **Re-point the pump** — `regen-fallback.mjs` target → `data/network.local.json` | DoD: running it leaves `git status` clean — **NOT started: coupled to the next item. Re-pointing alone leaves `fileStore.ts` serving a `network.json` nothing refreshes, so these two ship in the same increment.**
- [ ] [Max] Teach `fileStore.ts` to prefer `network.local.json` when present, resolved per-call | DoD: new test asserts local-wins and synthetic-fallback; 187 existing test files green
- [ ] [Max] De-PII the manifest — `participants` → `participantCount` + domains only | DoD: 0 email matches; 13 meetings still identifiable by title/date/link
- [ ] [Max] Redact the 3 prose leaks | DoD: Tier-B guard passes on all three
- [ ] [Max] Confirm the 110 `ARCHITECTURE-ATLAS.html` hits are SVG coordinates | DoD: 5 sampled, each inside `d=` or `viewBox`
...
Rides the existing `.githooks/pre-push` + CI vitest step — **a new test file is enforced at both gates with zero new wiring.** Gitleaks rejected: tuned for credential entropy, would sail past `christopher.acheson@gmail.com`, and `scripts/secrets-sweep.mjs` already covers credentials.
