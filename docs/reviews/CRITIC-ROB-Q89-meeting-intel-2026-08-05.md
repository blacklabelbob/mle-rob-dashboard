...
> **HALF SHIPPED 2026-08-05, Q89 inc.16 — and the review's own recommended half is the half that is REFUSED, with the reason.**
> **Shipped:** the `excerpt` now renders, as a quoted source line under the claim, on both surfaces (`meetingIntel.contextExcerpt` + `MeetingIntelSection`), suppressed when it is the claim restated so one source never reads as two. **57 of the 73 published item/excerpt pairs carry a line that says more than the claim; 16 correctly print nothing** — which is the review's own action-items observation, now enforced in code instead of being a caveat in prose.
> **Refused for now: the in-CRM anchor.** `/companies/C-2018#A-MTG-…` has no target. `TimelineEntry` (`lib/repSource.ts:67-71`) is `{type, summary, when}` — **it carries no activity id at all**, `ActivityTimeline` keys its rows on the array index, and the timeline is client-fetched, so the browser resolves the fragment before those rows exist. Stamping the URL today produces a link that silently lands you at the top of the page — a link to nothing is the same class of defect as a link to a lie, and this surface exists to prevent it. **The anchor half is therefore a prerequisite, not a rewrite:** widen `TimelineEntry` with the activity id, stamp `id=` on the row, then stamp `provenance.url`. Tracked as open under Q89.
> The DoD clause is *"can always be opened and checked."* Bringing the checked-against text onto the page satisfies the **checked** half completely and with no navigation at all; **opened** stays open. Tests: `lib/meetings/__tests__/contextExcerpt.test.ts` (5).
