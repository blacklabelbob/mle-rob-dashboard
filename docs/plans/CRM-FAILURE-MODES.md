...
| 5 | **Silent capture-workflow failure** (Gmail/AIDRE ingestion stops) | CRM quietly stops learning; looks fine, data is stale | **Task 3.6 in flight (inc.1 2026-07-22):** n8n Error Trigger → `POST /api/webhooks/n8n-error` → high flag on the ledger within seconds (covers run failures AND polling-trigger credential failures). CRM receiver built + tested; n8n wiring + forced-error live test remain | 🔨 receiver built |
