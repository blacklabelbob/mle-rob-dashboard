...
**Version:** 3.0.16 | **Created:** 2026-07-16 | **Updated:** 2026-07-21
**Status:** ACTIVE
**Owner:** Rob + Max
...
- [ ] Task 4.6 [Engineering] - RLS + roles implementing Task 1.13's matrix: `profiles` table (super_admin/management/sales_rep/sales_agent), book-of-business protection for sales_agent-imported contacts, policies per layer — rep tiers ship **gated-off** until reps exist | DoD: In test — sales_agent-scoped client cannot expose their protected book to management; rep cannot read another rep's unshared deal | ⚠️ KNOWN-OPEN RISK (critic-rob Q4b 7/21): prod `/api/network` is currently UNAUTHENTICATED — full graph incl. quoted $ + signed dates readable by anyone with the URL until an interim gate or this task lands (interim gate queued: BUILD-QUEUE Q10)
