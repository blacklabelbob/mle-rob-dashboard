...
### Added
- 2026-07-22 — **Rep CRM scaffold (Task 1b.3): "My Accounts" list + account workspace.** `/rep/accounts` — Attio-density, sortable (Priority / Quoted $ / Last touch) list of Jake's book, row click opens the workspace. `/rep/accounts/[id]` — header (status chip + quoted $ + one-tap Call/Email), "How they got here" source story, activity timeline (real feed via new read-only `/api/admin/activities`, gracefully empty until that table exists; hand-written demo history for DEMO leads with real history — Dale's stays an honest empty shell per his own "no contact yet" record), inline next-step widget, Phase-8 in-call action ghosts. "Rep View" nav gets a Today | My Accounts sub-nav. Shared helpers extracted to `lib/repSource.ts`; Phase-8 bar extracted to `components/PhaseEightBar.tsx`. No schema changes (next-step reuses the existing `relationship` column). Deployed, prod-verified all 6 accounts 200.
- 2026-07-22 — Fixed a 500 on every account workspace: an inline `format` callback was passed from the Server Component page straight into the client `InlineText` (functions aren't serializable across the RSC server→client boundary). Extracted `components/QuotedAmountInline.tsx` (a "use client" wrapper, same pattern `PersonEditor`/`PeopleTable` already use). Verified locally + against prod before redeploy.
- 2026-07-22 — **Critic Rob gate (72/100, REVISE) — punch fixes 1-9** (`docs/reviews/CRITIC-ROB-rep-scaffold-2026-07-22.md`): (1) `repMoney()` added for rep surfaces — exact sub-$100k ($9,500, not "$10k"; $27,500 pipeline, not "$28k"), `money()` untouched for admin rollups; (2) paid is now the apex everywhere — `touchReason()` checks `keyDates.paid` first ("client — paid", never "signed"/"quoted" on collected money), workspace header switches label to "collected" + emerald; (3) account workspace right column gets a visible/copyable Contact card (phone/email via the inline kit) + Key-dates chips (met/quoted/signed/paid), killing the desktop void; (4) global header nav no longer forces horizontal scroll at 390px viewport (`overflow-x-auto` + `min-w-0` + `shrink-0`, site-wide fix in `app/layout.tsx`); (5) `RepAccountListItem` DTO (`lib/repSource.ts`) — the accounts list now gets a mapped, minimal object (no `notes`, no `description`, no `estimate`/AI-revenue $) instead of the full `Person`, closing the RSC-payload leak; (6) `lib/__tests__/repSource.test.ts` added — 20 new tests covering `stageRank`, `touchReason` (incl. the paid branch), `sourceContext`, `lastTouchDate`, `repMoney`, and the DTO mapper (88/88 total); (8) `lastTouchDate()` now takes `max(keyDates, demo timeline)` so the list and the workspace timeline always agree (Rita: 7/11, not 7/10); (9) `components/DemoFooter.tsx` shared across `/rep`, `/rep/accounts`, and every workspace (incl. Dale's, which had no DEMO indicator at all before). Item 10 (alias rep client calls off `/api/admin/*`) logged as BUILD-QUEUE Q16, not built — Rob's explicit call, ahead of Phase-4 real reps.
