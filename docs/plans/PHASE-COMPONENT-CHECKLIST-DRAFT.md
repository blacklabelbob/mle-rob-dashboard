# Phase Component Checklists — DRAFT for Rob's edit
...
**Grounding:** dump 7.22.26-3 (verbatim rules) + the partner demo `https://mylocaleverything.com/app?demo=1` (fetched 2026-07-22). **Every item not directly confirmed by Rob's words or the demo is marked `[DRAFT — Rob confirm]`. No fabricated certainty.**
...
| `everything-agent` | Everything Agent active | The customer's core MLE agent running on their business. | [DRAFT — Rob confirm] demo has a dedicated "Everything Agent" tab + "7 agents active" |
| `social-connections` | Social channels connected | Their social accounts wired for posting/capture. | [DRAFT — Rob confirm] demo "Connections" tab + social channel connect/post section |
| `brand-knowledge` | Brand knowledge loaded | Voice/style/brand docs ingested so content sounds like them. | [DRAFT — Rob confirm] demo "brand knowledge management (voice/style documentation)" |
| `content-engine` | Content engine running | Weekly content planned + generated ("Generate my week"), library populated. | [DRAFT — Rob confirm] demo weekly content planning + content library/photo uploads |
| `social-radar` | Social Radar listening | Competitor/social feeds monitored for the customer's market. | [DRAFT — Rob confirm] demo "Social Radar" tab / market listening |
| `growth-scan` | AI Growth Scan delivered | Tech-stack scan complete — also the seed list for the P1→P2 "Top Automations" slot. | [DRAFT — Rob confirm] demo "AI Growth Scan for tech stack analysis" |
| `ai-receptionist` | AI receptionist live | Inbound calls answered 24/7. | [DRAFT — Rob confirm] NOT visible in the demo — inferred from the MLE voice roadmap; strike if P2 material |
...
Same per-customer slot structure as P2 (`p3-auto-<n>`), deeper scope. `[DRAFT — Rob confirm]` everything here: the demo gives only the framing ("deepest automation tier"); no component candidates are visible from outside, and dump 7.22.26-3 says only *"Same thing for Phase 3."* We deliberately draft **no** fake P3 items.

---

**Open for Rob's edit:** P1 strike/add/rename · P2/P3 slot count · whether `ai-receptionist` belongs in P1. Slugs freeze on your approval; names stay editable forever.
